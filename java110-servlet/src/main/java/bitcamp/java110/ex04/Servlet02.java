...
public class Servlet02 extends GenericServlet {
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        // => httl://localhost:8888/ex04/post.html 페이지에서 값을 입력한 후 보내기 버튼 클릭
        // 
        // => 그러나 서블릿에서 값을 꺼내는 방법은 GET 요청 때와 같다.
        
        String name = req.getParameter("name");
        int age = Integer.parseInt(req.getParameter("age"));
        boolean working = Boolean.parseBoolean("working");
...
// POST 요청의 특징
// #message body영역에 데이터를 포함시킨다.
//  => URL에 데이터가 포함되어 있지 않다.
//  => 즐겨찾기에는 URL만 저장되기 떄문에 데이터가 포함되지 않은 URL을 저장해봐야 소용없다.
//  => 데이터 조회 화면을 요정할 때는 POST 방식을 사용하지 않는다.
//
// #보안
//  => URL에 데이터가 포함되어 있지 않기때문에 
//     데이터가 노출되지 말아야 할 경우(예를 들면 로그인이나 회원가입 등)
//     POST방식이 적합하다.
//
// #대용량 데이터
//  => 대부분의 웹 서버는 request-uri 또는 헤더를 포함한 메타 데이터의 크기를
//     8KB 미만으로 설정하고 있다.
//     그러나 message body 영역은 제한ㅇ르 두지 않거나 큰 용량을 허락하고 있다.
//  => 그래서 대용량 데이터를 뵤낼 때는 POST방식을 사용한다.
//
// #바이너리 데이터 전송
...
//  => 게시글 등록 /변경 처럼 대용향의 데이터를 전송해야 하는 경우
