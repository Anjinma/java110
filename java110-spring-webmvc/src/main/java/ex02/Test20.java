...
public class Test20 {
    ...
    // @Autowired는 스프링 전용 태그고
    // 자바표준 태그인 @Resource나 @Inject를 권장한다. 

    /*
     테스트:
          httl://localhost:8888/ex02/Test20.html

     주의!
     프론트 컨트롤러가 멀티파트 데이터를 처리하려면,
     방법1) DD 파일에 프론트 컨트롤러를 선언할 떄 다음의 태그를 설정 한다.
            <multipart-config>
     ...
     방법2) Apache common fileupload를 이용하여 멀티파트 데이터를 처리하는 
            스프링 제공 객체를 추가하면 된다.
      => multipartResolver라는 이름으로 다음과 같이 객체를 등록해야 한다. 
