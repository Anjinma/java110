// 커스텀 프로퍼티 에디터 - 페이지 컨트롤러에 커스텀 프로퍼티 장착하기
package ex02;
...
public class Test15 {
    ...
    // 콜라이언트가 보낸 데이터는 메서드의 파라미터 타입에 맞춰서 
    // 변환하여 전달한다.
    ...
    // => 프론트컨트롤러가 request handler를 호출하기 전에
    //    클라이언테가 보낸 데이터를 request handler의 파라미터 타입으로 바꾼다.
    // => 프론트컨트롤러가 데이터 타입을 바꿀때 WebDataBinder를 통해 설정된
    //    도구를 사용한다.
    // => @InitBinder는 프론트 컨트롤러가 WebDataBinder르 ㄹ사용하기 전에
    //    이 애노테이션이 지정되 ㄴ메서드를 호출하여 먼저 초기화 시킨 다음에
    //    사용할 것을 요구하는 애노테이션이다.
    ...
    public void initBinder(WebDataBinder binder) {
        ...
        format.setLenient(false);// 24시간으로 표시할거지 여부
        
        // String ==> java.util.Date 변환기 등록
        binder.registerCustomEditor(
                Date.class, 
                //new DatePropertyEditor(format) 개발자가 만든것을 써도 되고!
                new CustomDateEditor(format,true)); // 스프링에서 제공하는 것을 써도 되고!
...
// 클라리언트가 보낸 데이터가 리퀘스트 핸들러의 탑입으로 변환하지 못바꿀때
// 2018-10-24(string)을 Day로 못바꿀때뜨는 에러 메시지
/*
